      ...
      tap((event) => {
        if (event instanceof HttpResponse) {
          if (event.headers.has('X-nuvem-alert')) {
            this.notification.addSuccessMessage(
              event.headers.get('X-nuvem-alert')
            );
          } else if (
            (event.status === STATUS.OK || event.status === STATUS.CREATED) &&
            req.method != 'GET'
          ) {
            this.notification.addSuccessMessage(
...
export const httpInterceptorProviders = [
  {
    ...
    useClass: SuccessMessageHandlerInterceptor,
    multi: true
  }
];
